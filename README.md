...
本项目创建于2020年9月1日，最近的一次更新时间为2022年3月27日。本项目会持续更新，直到海枯石烂。
...
- [x] 关键字：关键字是一些特殊的用来帮助编译器理解和解析源代码的单词。Go语言中有**25个**关键字或保留字。关键字只能用在程序语法允许的地方，不能作为名称使用。
...
- [x] 程序语言的常量：
  ...
  - 常量又称常数，是一个在程序正常执行过程中不会被程序改变的值，也就是说，这个值是恒定的。常数可以分为不同的数据类型。
