...
- [x] Go语言函数介绍：
  ...
  - 在Go语言中，函数是一等公民类型。换句话说，我们可以把函数当作值来使用。
  - 函数是基本的代码块，用于执行一个任务。Go语言最少需要有 main() 函数。
  - main()函数是在启动后第一个执行的函数（如果有 init() 函数则会先执行该函数）。main()函数没有参数，也没有返回类型。
  - Go语言是编译型语言，函数编写的顺序是无关紧要的。但鉴于可读性的需求，最好把main()函数写在文件的前面，其他函数按照一定逻辑顺序进行编写（例如函数被调用的顺序）。
...
- [x] Go内置函数介绍：
  ...
  - 内置函数和自定义函数有很多差别。其中一个差别是很多内置函数支持范型参数，但自定义函数不支持（至少到Go 1.15为止）。 更多这两者之间的差异将在下面逐一提及。
