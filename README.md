...
- [x] Go语言指针介绍：
  ...
  - 虽然Go语言吸收融合了很多其语言中的各种特性，但是Go主要被归入C语言家族。其中一个重要的原因就是Go语言和C语言一样，也支持指针。但Go语言中的指针相比C语言指针有很多限制。
  - 指针是Go中的一种类型分类（kind）。一个指针是某个指针类型的一个值。一个指针可以存储一个内存地址。我们经常称一个指针为一个内存地址，一个内存地址为一个指针。 一个指针中存储的内存地址为另外一个值的地址。
